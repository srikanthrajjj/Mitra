import { Briefcase, CheckCircle2, ChevronRight, Mail, MessageSquare } from 'lucide-react';
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 flex-1 space-y-1">
          <div className="flex items-center gap-1.5">
            <Briefcase className="h-3 w-3 shrink-0 text-violet-400/90" />
  ...
  return (
    ...
    >
      ...
      >
        ...
        {review.completedLabel ?? (isApproved ? 'Approved' : 'Declined')}
      </span>
    </button>
  );
}
    ...
    <div className="stakeholder-sidebar flex h-full flex-col px-3 py-4">
      <div className="mb-4 px-1">
        ...
        {pending.length > 0 && (
          <p className="mt-2 text-[12px] leading-relaxed text-muted-foreground">
            ...
            {' '}executive summar{pending.length === 1 ? 'y' : 'ies'} ready for your desk.
          </p>
        )}
      </div>
      ...
      <div className="flex-1 space-y-5 overflow-y-auto">
        ...
        {pending.length === 0 && completed.length === 0 && (
          <p className="px-1 text-[12px] text-muted-foreground">No executive summaries in your queue.</p>
        )}
      </div>

      <div className="mt-4 rounded-xl border border-sidebar-border/50 bg-sidebar-accent/20 p-3">
        <p className="mb-2 text-[10px] text-muted-foreground">Need a briefing call?</p>
        <Button
          variant="outline"
